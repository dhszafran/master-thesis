...
margins, dydx(female)
marginsplot, recast(scatter) xscale(range(-0.5 2.5)) title(" ") yline(0) xtitle("Accuracy of subjective weight estimation relative to actual BMI", size(medsmall) margin(t=2)) ytitle("Effects on Probability", size(medsmall) margin(r=2)) xlabel(0 "Accurate estimation" 1 "Overestimation" 2 "Underestimation", notick labsize(small)) ylabel(, notick labsize(small) angle(0)) plot1opts(msymbol(O) msize(medsmall) mcolor(green)) ci1opts(lcolor(green))

generate boys = .
replace boys = 0 if female == 1
replace boys = 1 if female == 0
